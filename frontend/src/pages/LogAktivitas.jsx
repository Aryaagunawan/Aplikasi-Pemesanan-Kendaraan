...
export default function LogAktivitas({ logs }) {
    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Log Aktivitas Sistem</h2>
                <p className="text-slate-500 mt-1">Jejak rekaman aktivitas pengguna aplikasi.</p>
            </div>
                            ...
                            <div key={log.id} className="relative pl-8">
                                ...
                                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 hover:shadow-md transition-shadow">
                                    <p className="text-sm text-slate-800">
                                        <span className="font-extrabold text-indigo-600 mr-2">{log.user}</span>
                                    ...
                                    <p className="text-xs font-bold text-slate-400 mt-2 flex items-center">
                                        <Activity size={12} className="mr-1" /> {log.timestamp}
                                    </p>
                                </div>
                            </div>
                        ))}
